...
# -*- coding: utf-8 -*-
"""
Model t4 uses the the state data as parameterized by t, as well as a new state
data representing a "carry-over" effect for the dissatisfaction factor. With
this addition, an action taken at a time period will affect the future states,
something which needed to be in the model but was not presented in the original
paper.
...
# MODEL PARAMETERS
# newly added "carry-over" coefficient for the dissatisfaction
cocoef = 0.9
...
def econscurt(t,n,retprice):
    return max(0, edemandcurt(t,n) * (1 + elasticity(t) * ((retprice - wholeprice(t)) / wholeprice(t))))
